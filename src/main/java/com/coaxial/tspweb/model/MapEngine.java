...
/**
 * This Class manages all connections to the Google APIs.
 */
public class MapEngine
{
    //private static String key = "AIzaSyCY1PqwBDt9nA2YykeLA-u4XMlrGsyCRVc"; //The google api key
    ...
    /**
     * Translates the given Coordinates to Strings, then calls {@link MapEngine#getDistances(String[], TravelMode)}.
     *
    ...
    /**
     * Request the distance matrix of the given locations from the Google Distance Matrix API.
     * Due to API limitations, large requests are splitted into 2 single requests.
     *
     * @param locations the locations for which the distance matrix should be requested
     * @param mode      the travel mode
    ...
    {
        DistanceMatrixApiRequest req = DistanceMatrixApi.getDistanceMatrix(context, locations.length > 10 ? Arrays.copyOfRange(locations, 0, Math.round(locations.length / 2)) : locations, locations)
                .mode(mode);
        DistanceMatrixApiRequest req2 = locations.length > 10 ? DistanceMatrixApi.getDistanceMatrix(context,
                Arrays.copyOfRange(locations, Math.round(locations.length / 2), locations.length), locations).mode(mode) : null;
        try
        {
            DistanceMatrix mat = req.await();
            if (req2 != null) Thread.sleep(1000);
    ...
    /**
     * Queries the elevation values form The Google Elevation API ofr the given locations.
     *
    ...
    public static double[] getElevations(LatLng[] places)
    {
        ...
        try
        {
            return Arrays.stream(pending.await())
                    .sequential()
                    .mapToDouble(r -> r.elevation)
                    .toArray();
