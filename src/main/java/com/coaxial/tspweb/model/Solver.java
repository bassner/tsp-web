...
/**
 * Responsible for all solving processes. Completely .
 */
public class Solver
{
    ...
    /**
     * Solves the Travelling Salesman Problem. For details on how the algorithm works, please refer to the
     * PDF-Documentation.
     *
     * @param request       the client request
     * @param sessionWorker the SessionWorker calling this method
                                   ...
                                   SessionWorker sessionWorker)
    {
        ...
        sessionWorker.tellStatus(StatusCode.CALCULATING, "Frage Distanzmatrix ab...");
        DistanceMatrixElement[][] distances = MapEngine.getDistances(places, TravelMode.DRIVING);
        if (distances == null)
        {
            sessionWorker.tellStatus(StatusCode.ERROR, "Abfragen der Distanzmatrix fehlgeschlagen.");
            return null;
        }

        sessionWorker.tellStatus(StatusCode.CALCULATING, "Frage Höheninformationen ab ...");
        double[] elevations = MapEngine.getElevations(places);
        if (elevations == null)
        {
            sessionWorker.tellStatus(StatusCode.ERROR, "Abfragen der Höheninformationen fehlgeschlagen.");
            return null;
        }

        sessionWorker.tellStatus(StatusCode.CALCULATING, "Berechne Verbrauchsinformationen ...");
        ...
        Thread updater = new Thread(() ->
        {
            while (!Thread.interrupted())
            {
                sessionWorker.tellStatus(StatusCode.CALCULATING,
                        "Berechne Pfad mit besten Verbrauchswerten...",
