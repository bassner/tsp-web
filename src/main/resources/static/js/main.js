let geocodes; // a globaly array, representing the locations currently entered in the fields
let map; //the google maps map object
let geocoder; //the google maps geocoder object
let ws; //the current websocket (connection to server)
let directionsService; //the google directions service object
 ...
 * This callback is also used to initialize the location text fields, since some browsers may store old field values
 * that have to be parsed again on a reload; this can only be done as soon as the map finished loading.
 */
function initMap() {
        ...
        .click(() => {
            ...
            ws.onmessage = data => {
                ...
                //if calculation done and result retrieved
                    ...
                    $('#loading-header').text("Erstelle Maps-Route...");
                    ...
                    createDirections(msg.path, success => {
                        ...
                        if (!success) {
                            onLoadingError(bounceInterval, "Fehler beim Laden der Karte!");
                            return;
                        }
                        ...
                        $('#loading-header').text("Berechnung abgeschlossen!");
